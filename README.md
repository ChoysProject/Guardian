...
왜 만들었는지, 화면 흐름, 서버 등록, 플러그인, AI 총평이 붙는 시점은 [SYSTEM.md](SYSTEM.md) 를 봅니다.
...
3. **서버 → 리소스 및 성능 수집 대상 서버**에 서버를 등록합니다. 검색창 오른쪽 **전체 수집하기** 또는 행의 **수집**으로 가져오거나, JSON을 직접 넣습니다.
4. **보고서 → 리소스 및 성능 보고서**에서 설명 오른쪽 **전체 보고서 생성**을 누르거나, 서버 박스를 눌러 목록을 고르면 상세가 열립니다.
